...
const errorHandlerMiddleware = (store) => (next) => async (action) => {
  if (action.error) {
    ...
    if (action.payload?.statusCode === 401 && localStorage.getItem(REFRESH_TOKEN)) {
...
export const setupStore = () =>
  configureStore({
    ...
    middleware: (getDefaultMiddleware) => getDefaultMiddleware({ serializableCheck: false }).concat(errorHandlerMiddleware),
  });
...
export type AppThunk<T = void, A = unknown> = ThunkAction<Promise<T>, RootState, A, Action<string>>;
